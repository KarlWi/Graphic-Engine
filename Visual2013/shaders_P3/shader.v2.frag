...
vec3 shade()
{
	...
	for(int i = 0; i<nPointLight;i++){
		...
		vec3 diffuse = Kd*color*dot(N,L)*pointLightIntensity[i];
		c+=clamp(diffuse,0.0,1.0);
		vec3 V = normalize (-pos);
		vec3 R = normalize (reflect (-L,N));
		float factor = max (dot (R,V), 0.01);
		vec3 specular = Is*Ks*vec3(1.0)*pow(factor,alpha);
	...
	for(int i = 0; i<nDirectionalLight;i++){
		...
		vec3 diffuse = Kd*dot(N,L)*directionalLightIntensity[i];
		c+=clamp(diffuse,0.0,1.0);
	}
	...
	for(int i = 0; i<nFocalLight;i++){
		...
		if(dot(focalLightDir[i],-L) > cos(focalLightAperture[i])){
			diffuse = Kd*dot(N,L)*focalLightIntensity[i]*((focalLightDir[i]*(-L) -cos(focalLightAperture[i]))/(1-cos(focalLightAperture[i])));
		}
		//else{
		//	diffuse = vec3(0.0);
		//}
